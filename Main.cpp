...
enum ACTIONTYPE {
    ...
    MEMBER_READ,
    MEMBER_SEARCH_TITLE,
    MEMBER_SEARCH_SERIAL,
    BORROW_LIST,
    BORROW_BOOK,
    RETURN_BOOK,
    SHOW_COLLECTION,
    SHOW_BOOK_LIST_COLLECTION,
...
int main() {
    ...
    // Default user:
    Member *m =
        new Member(1, "dylan245", "12345", 14, "0906159559", "Pham Thanh Dat");
    ...
    // General menu
    Menu *logOut =
        new Menu("LOG OUT", "Thank you for using our service!", LOGOUT);
    Menu *registerReadMenu =
        new Menu("READING SECTION FOR REGISTERED USER", "Read Book");
    Menu *exitMenu = new Menu("EXIT LIBRARY", "Bye!", EXIT);

    // Main Menu
    Menu *mainMenu = new Menu("ONLINE LIBRARY", "Guess Loby");
    Menu *guestReadMenu =
        new Menu("READING SECTION FOR GUESS", "Read Book", GUEST_READ);
    Menu *guestSearchMenu =
        new Menu("GUESS SEARCHING BOOK", "Choose kind of method to search");
    Menu *guestSearchTitleMenu =
        new Menu("SEARCHING BOOK BY TITLE", "Please enter book title",
                 GUEST_SEARCH_TITLE);
    Menu *guestSearchSerialMenu =
        new Menu("SEARCHING BOOK BY SERIAL", "Please enter book serial",
                 GUEST_SEARCH_SERIAL);
    ...
    Menu *userRegisterMenu =
        new Menu("USER REGISTER", "Please enter your information", REGISTER);
    ...
    Menu *loginMemberMenu =
        new Menu("MEMBER LOGIN", "Please login", LOGIN_MEMBER);
    ...
    Menu *collectionMenu =
        new Menu("COLLECTION SECTION", "List of collections", SHOW_COLLECTION);
    Menu *showListBookCollectionSubcribe =
        new Menu("BOOK LIST COLLECTION SUBSCRIBE",
                 "List of books in collection", SHOW_BOOK_LIST_COLLECTION);
    ...
    Menu *memberSearchMenu =
        new Menu("MEMBER SEARCHING SECTION", "Choose kind of method to search");
    Menu *memberSearchTitleMenu =
        new Menu("SEARCHING BOOK BY TITLE", "Please enter book title",
                 MEMBER_SEARCH_TITLE);
    Menu *memberSearchSerialMenu =
        new Menu("SEARCHING BOOK BY SERIAL", "Please enter book serial",
                 MEMBER_SEARCH_SERIAL);
    ...
    Menu *borrowListMenu =
        new Menu("BORROW LIST", "list of borrowed books", BORROW_LIST);
    ...
    Menu *subscribedCollectionMenu =
        new Menu("SUBSCRIBE COLLECTION", "Member subscribes collection",
                 SUBSCRIBE_COLLECTION);
    Menu *unSubsribedCollectionMenu =
        new Menu("UNSUBSCRIBE COLLECTION", "Member unsubscribes collection",
                 UNSUBSCRIBE_COLLECTION);
    ...
    Menu *createAdminMenu =
        new Menu("CREATE ADMIN", "Create admin", CREATE_ADMIN);
    Menu *adminReadMenu =
        new Menu("READING ADMIN SECTION", "Read Book", ADMIN_READ);
    Menu *adminSearchMenu =
        new Menu("ADMIN SEARCHING SECTION", "Choose kind of method to search");
    Menu *adminSeachTitle =
        new Menu("SEARCHING BOOK BY TITLE", "Please enter book title",
                 ADMIN_SEARCH_TITLE);
    Menu *adminSearchSerial =
        new Menu("SEARCHING BOOK BY SERIAL", "Please enter book serial",
                 ADMIN_SEARCH_SERIAL);
    ...
    Menu *manageBookMenu =
        new Menu("MANAGEMENT BOOK SECTION", "Choose manage action!");
    ...
    Menu *manageCollectionMenu =
        new Menu("MANAGEMENT COLLECTION SECTION", "Choose manage action!");
    Menu *createCollectionMenu =
        new Menu("CREATE COLLECTION", "Create collection", CREATE_COLLECTION);
    Menu *deleteCollectionMenu =
        new Menu("DELETE COLLECTION", "Delete collection", DELETE_COLLECTION);
    Menu *addBookCollection =
        new Menu("ADD BOOK TO COLLECTION", "Add book to collection",
                 ADD_BOOK_COLLECTION);
    Menu *removeBookCollection =
        new Menu("REMOVE BOOK FROM COLLECTION", "Remove book from collection",
                 REMOVE_BOOK_COLLECTION);
    ...
    while (true) {
        ...
        case REGISTER: {
            currentMenu->displayMenu();
            char ch;
            string registerName, registerPhone, registerUserName,
                registerPassword;
            ...
            g.inviteRegister(1, registerUserName, registerPassword, id,
                             registerPhone, registerName);
            cout << "Register success! Press  any key to continue: ";
            cin.get(ch);
            currentMenu = currentMenu->getParentMenu();
        } break;
        ...
        case LOGIN_MEMBER: {
            ...
            if (currentMember) {
                ...
                cout << "Invalid username or password! Press any key to "
                        "continue: ";
                cin.get(ch);
                currentMenu = currentMenu->getParentMenu();
            }
        ...
        case MEMBER_READ: {
            ...
            cout << "Enter book serial you want to read: ";
            bookSerial = getValidSerial();
        ...
        case SHOW_BOOK_LIST_COLLECTION: {
            ...
            cout << "Enter collection ID you want to see its list of books: ";
            cin >> collectionID;
            int index =
                currentMember->getSubscribeCollectionIndexById(collectionID);
            if (index != -1) {
                for (Book *b : currentMember->getSubscribedCollections()[index]
                                   ->getBooks()) {
                    b->printInfo();
                }
            } else {
        ...
        case BORROW_BOOK: {
            ...
            string bookSerial;
            cin.ignore();
            cout << "Enter book serial you want to borrow: ";
            bookSerial = getValidSerial();
        ...
        case RETURN_BOOK: {
            ...
            string bookSerial;
            cin.ignore();
            cout << "Enter book serial you want to return: ";
            bookSerial = getValidSerial();
        ...
        case SUBSCRIBE_COLLECTION: {
            ...
            cout << "Enter collection ID you want to subscribe: ";
            cin >> collectionID;

            if (library.getCollectionById(collectionID) != nullptr) {
                currentMember->subscribe(
                    library.getCollectionById(collectionID));
        ...
        case UNSUBSCRIBE_COLLECTION: {
            ...
            cout << "Enter collection ID you want to unsubscribe: ";
            cin >> collectionID;

            int index =
                currentMember->getSubscribeCollectionIndexById(collectionID);
            if (index != -1) {
                if (currentMember->getSubscribedCollections()[index] ==
                    library.getCollectionById(collectionID)) {
                    currentMember->unSubscribe(
                        library.getCollectionById(collectionID));
        ...
        case CREATE_ADMIN: {
            ...
            cout << "Phone number: ";
            getline(cin, adminPhone);
            cout << "User Name: ";
            getline(cin, adminUserName);
            cout << "Password: ";
            getline(cin, adminPassword);

            currentAdmin->createAdmin(1, adminName, adminPhone, id,
                                      adminUserName, adminPassword);
            cout << "Create admin success! Press any key to continue: ";
            cin.get(ch);
            currentMenu = currentMenu->getParentMenu();
        }
        ...
        case ADD_BOOK: {
            ...
            cout << "Enter number of book page: ";
            cin >> numBookPage;
            cout << "Enter number of free page: ";
            cin >> numFreePage;
            Book *newBook = new Book(bookTitle, bookSerial, bookAuthor,
                                     bookCategory, numBookPage, numFreePage);
        ...
        case HIDE_BOOK: 
       {
        ...
        case EDIT_BOOK: {
            ...
            cin.ignore();
            cout << "Enter book serial you want to edit: ";
            bookSerial = getValidSerial();
            cout << "Enter number of book page: ";
            cin >> numBookPage;
            cout << "Enter number of free page: ";
        ...
        case DELETE_COLLECTION: {
            ...
            cout << "Enter collection id you want to delete: ";
            cin >> collectionId;
            if (library.getCollectionById(collectionId) != nullptr) {
                currentAdmin->deleteCollection(
                    library.getCollectionById(collectionId));
        ...
        case ADD_BOOK_COLLECTION: {
            ...
            cout << "Enter collection id: ";
            cin >> collectionId;
            if (library.getCollectionById(collectionId) != nullptr) {
                cin.ignore();
                cout << "Enter book serial: ";
                bookSerial = getValidSerial();
                currentAdmin->addBookCollection(
                    library.getBookBySerial(bookSerial),
                    library.getCollectionById(collectionId));
        ...
        case REMOVE_BOOK_COLLECTION: {
            ...
            if (library.getCollectionById(collectionId) != nullptr) {
                ...
                cout << "Enter book serial: ";
                bookSerial = getValidSerial();
                currentAdmin->removeBookCollection(
                    library.getBookBySerial(bookSerial),
                    library.getCollectionById(collectionId));
